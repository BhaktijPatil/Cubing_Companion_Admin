...
public class ResultRoundStatusAdapter extends RecyclerView.Adapter<ResultRoundStatusAdapter.MyViewHolder>{

    private CompetitionEvent event;
    private ResultEventsActivity.ClickListener clickListener;

    private Context context;

    @NonNull
    @Override
    public ResultRoundStatusAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_row_result_event_round, parent, false);
        context = parent.getContext();
        return new ResultRoundStatusAdapter.MyViewHolder(itemView, clickListener);
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull ResultRoundStatusAdapter.MyViewHolder holder, int position) {
        ...
        if(Calendar.getInstance().getTimeInMillis() > competitionEventRound.startTime.getSeconds()*1000)
        {
            // Start new activity to load results
            holder.resultHolderCardView.setOnClickListener(v -> {
                Intent resultIntent = new Intent(context, ResultDetailsActivity.class);
            ...
            if(Calendar.getInstance().getTimeInMillis() < competitionEventRound.endTime.getSeconds()*1000) {
                // Round is Live
            ...
            holder.resultStatusTextView.setTextColor(context.getColor(R.color.textPrimary));
    ...
    static class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener{
        ...
        private WeakReference<ResultEventsActivity.ClickListener> listenerRef;

        MyViewHolder(@NonNull View itemView, ResultEventsActivity.ClickListener listener) {
            super(itemView);
    ...
    ResultRoundStatusAdapter(CompetitionEvent event, ResultEventsActivity.ClickListener clickListener){
        this.event = event;
